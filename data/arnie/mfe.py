...
def mfe(seq, package='vienna_2', T=37,
    ...
    if pkg=='vienna':
        if linear:
            ...
            struct, mfe, fe_ens, fq_mfe_ens = mfe_vienna_(seq, version=version, T=T, dangles=dangles, constraint=constraint, motif=motif, param_file=param_file,
    ...
    if return_dG_MFE:
        ...
        return struct, mfe, fe_ens, fq_mfe_ens
...
def mfe_vienna_(seq, T=37, version='2', constraint=None, motif=None, param_file=None, dangles=True, reweight=None,
    ...
    if 'omitting constraint' in stderr.decode('utf-8'):
        ...
        struct = decoded[1].split(' ')[0]
        try:
            mfe = float(re.sub("[() ]","", decoded[1].replace(struct,'')))
            fe_ens = float(decoded[2].split('=')[-1].lstrip().split(' ')[0]) #float(decoded[2].split(' ')[-2])
            fq_mfe_ens = float(re.sub(';','',decoded[3].split(' ')[-1]))
        except:
            print(decoded)
