...
def run_model(data, file):
    ...
    for m in available_models.keys():
        ...
        for mf in available_models[m]['capsnets'].keys():
            ...
            out = out.drop(['guides', 'starts', 'ends', 'strands', 'guides_and_pams', 'PAMs'], axis=1)

            out['Test_file'] = file
            out['Trained_data'] = model_name
            out['Model_arch'] = fm

            out.to_csv(f'{dir}/{model_name}/{fm}/{model_name}_{fm}_ontarget.tsv', sep='\t', index=False)
